...
const NAV_HELPERS: Record<string, string> = {
  "/dashboard": "Επιχειρησιακή εικόνα ημέρας",
  "/children": "Μητρώο ωφελούμενων & πρόγραμμα",
  "/parents": "Γονείς, κηδεμόνες, επικοινωνία",
  "/staff": "Θεραπευτές & διοικητική ομάδα",
  "/schedule": "Ατομικές & ομαδικές συνεδρίες",
  ...
  const renderSection = (label: string, sectionItems: typeof items) => {
    ...
    return (
      <div className="space-y-1">
        <p className="px-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">{label}</p>
      ...
      >
        <div className="flex h-[4.25rem] shrink-0 items-center border-b border-border bg-gradient-to-r from-surface-card to-surface-muted/30 px-4">
          <Link href="/dashboard" className="min-w-0" onClick={() => setOpen(false)}>
            ...
            <span className="mt-0.5 block truncate font-semibold tracking-tight text-ink">Λειτουργία κέντρου</span>
